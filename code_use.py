...
UPGRADE_CONFIG = {
    ...
    "upgrade_p6": {
        "title": "Max HP",
        "costs": [300, 500, 780, 1500, 2450, 4500, 7500, 9800, 11000,
                  13580, 16050, 20040, 25100, 30000, 37500],
    ...
    "upgrade_p7": {
        ...
        "skills": [
            {"time": 10, "hp": 0}, {"time": 10, "hp": 1}, {"time": 8, "hp": 1},
            {"time": 8, "hp": 2}, {"time": 7, "hp": 2}, {"time": 7, "hp": 3},
            {"time": 6, "hp": 3}, {"time": 5, "hp": 3}, {"time": 5, "hp": 4},
            {"time": 4, "hp": 4}
        ],
        "skill_desc": "{}"
    },
    "upgrade_p8": {
        "title": "Invincible",
        "costs": [700, 1000, 1200, 1400, 1700, 2300, 3700, 4500,
                  5700, 7600, 9800, 12000, 15800, 21400, 25780],
        "skills": [1000, 1200, 1400, 1600, 1800, 2000, 2200, 2400,
...
current_levels = {
    f"upgrade_p{i}": 0 for i in range(1, len(UPGRADE_CONFIG) + 1)
}
...
player_skins = {     #修正字典載入
    "red"        : {"color": tool.Colors.RED,        "value":    0,  "has_bought":  True, "effect":                                            "none", "power":               1},
    "orange"     : {"color": tool.Colors.ORANGE,     "value":  500,  "has_bought": False, "effect":                                     "player_size", "power":             0.9},
    "dark orange": {"color": tool.Colors.ORANGE2,    "value": 1400,  "has_bought": False, "effect":          ["points_multiplier", "max_hp", "speed"], "power": [2.3, 0.7, 0.7]},
    "yellow"     : {"color": tool.Colors.YELLOW,     "value":  900,  "has_bought": False, "effect":                    ["points_multiplier", "speed"], "power":      [1.7, 0.8]},
    "green"      : {"color": tool.Colors.GREEN,      "value":  800,  "has_bought": False, "effect":                               ["max_hp", "speed"], "power":      [1.2, 0.7]},
    "light blue" : {"color": tool.Colors.CYAN,       "value": 1400,  "has_bought": False, "effect":                ["coin_multiplier", "player_size"], "power":      [1.9, 0.8]},
    "blue"       : {"color": tool.Colors.BLUE,       "value":  900,  "has_bought": False, "effect":                                    "enemy_damage", "power":             0.7},
    "purple"     : {"color": tool.Colors.PURPLE,     "value":  950,  "has_bought": False, "effect":                  ["purple_enemy_damage", "speed"], "power":      [0.2, 0.9]},
    "pink"       : {"color": tool.Colors.PINK,       "value": 1200,  "has_bought": False, "effect":               ["speed", "points_coin_multiplier"], "power":      [1.8, 0.9]},
    "white"      : {"color": tool.Colors.WHITE,      "value":  720,  "has_bought": False, "effect":                    ["speed", "points_multiplier"], "power":      [1.3, 1.2]},
    "gray"       : {"color": tool.Colors.GRAY,       "value":  750,  "has_bought": False, "effect":                      ["invincible_time", "speed"], "power":      [1.5, 1.1]},
    "black"      : {"color": tool.Colors.BLACK,      "value":  700,  "has_bought": False, "effect":                   ["black_enemy_damage", "speed"], "power":      [0.2, 0.8]},
    "gold"       : {"color": tool.Colors.GOLD,       "value": 3500,  "has_bought": False, "effect":          ["coin_multiplier", "points_multiplier"], "power":        [4, 1.5]},
    "brown"      : {"color": tool.Colors.BROWN,      "value": 2500,  "has_bought": False, "effect":                   ["enemy_spawn_speed", "max_hp"], "power":      [1.7, 1.2]},
    "dark green" : {"color": tool.Colors.DARK_GREEN, "value": 2100,  "has_bought": False, "effect":                               ["max_hp", "speed"], "power":      [2.0, 1.2]},
}
    ...
    for i in range(1, len(UPGRADE_CONFIG) + 1):
        ...
        if key in UPGRADE_CONFIG:
            ...
            if is_max:
                ...
                display_text = prefix + f"Cost: ${costs[lvl]}"
                display_color = p_colors[i-1]    # 沒滿級使用專屬顏色
            
            # --- 存入字典 ---
            upgrade_hub_layout[key] = {
                "title": display_text,
                "color": display_color
            }
...
def load_data():
    ...
    global total_points, current_levels, \
           longest_survived_time, player_skins, now_player_skin, current_player_color_name, \
           game_state, gm_i, has_buy_crazy
...
def load_resets():
    global \
        mode_button_color, next_spawn_range, mode_speed_buff, gm_points_buff, game_mode, g_m, gm_i, save_mode, s_m, sm_i, spawn_time_debuff, \
        now_p1_skill, now_p2_skill, now_p3_skill, now_p4_skill, \
        now_p5_skill, now_p6_skill, now_p7_skill, now_p8_skill, \
        enemy_damage_buff
...
def reset_game():
    global \
        player_rect, player_size, player_color, player_speed, current_player_speed, \
        enemy_list, treasures, treasure_points, next_spawn_range, points, \
        mode_speed_buff, gm_points_buff, \
        maybe_cheat, from_pause, \
        start_button_color, start_button_text, start_button_text_color, \
        mode_button_color, \
        last_hit_time, \
        player_hp, player_max_hp, \
        countdown_text, countdown, passed_time, \
        coin_chance, now_treasure, \
        last_cure_time, \
        has_plus_points, has_save_survived_time, \
        countdowning, invincible_duration, now_p8_skill, \
        clicked_key, \
        afk_timer, last_player_pos, AFK_LIMIT
    ...
    player_color, player_speed, current_player_speed = now_player_skin, (5 + now_p1_skill) * player_speed_buff, (5 + now_p1_skill) * player_speed_buff
...
def coin_rect():
    global total_points, WIDTH
    display_points = "%g" % round(total_points, 1)
    #把背景調成一個有釘釘子的木塊
    tool.text_button(f"${display_points}", tool.Colors.BLACK, tool.Colors.GOLD, WIDTH - 110, 20, 100, 40, size=22, font_type="")
...
while running:    
    ...
    if game_state == "menu":
        ...
        tool.show_text("settings", tool.Colors.WHITE, 40, 70, size=24, font_type="")          #--|-----版面改好後顯示
        if right_img_loaded:                                                                  #--|
        ...
        start_button = tool.text_button(start_button_text, start_button_text_color, start_button_color, 0, 220, 300, 70, b_center=True)
        settings_button = tool.text_button("Settings", settings_button_text_color, settings_button_color, WIDTH // 2 - 150, 310, 140, 70)
        upgrade_button = tool.text_button("Upgrades", tool.Colors.BLACK, upgrade_button_color, WIDTH // 2 + 10, 310, 140, 70)
    ...
    #難易度與最長存活時間
        ...
        coin_rect()
        tool.show_text("Difficulty And Longest Servided Time", tool.Colors.WHITE, 0, 80, size=34, screen_center=True)
        ...
        easy_rect = tool.CR(pygame.Rect(70, 210, 500, 50), tool.Colors.GREEN, show=(game_mode=="easy"));easy_rect.draw(screen)
        normal_rect = tool.CR(pygame.Rect(70, 270, 500, 50), tool.Colors.YELLOW, show=(game_mode=="normal"));normal_rect.draw(screen)
        hard_rect = tool.CR(pygame.Rect(70, 330, 500, 50), tool.Colors.ORANGE, show=(game_mode=="hard"));hard_rect.draw(screen)
        super_hard_rect = tool.CR(pygame.Rect(70, 390, 500, 50), tool.Colors.RED, show=(game_mode=="super_hard"));super_hard_rect.draw(screen)
        crazy_rect = tool.CR(pygame.Rect(70, 450, 500, 50), tool.Colors.PURPLE, show=(game_mode=="crazy"));crazy_rect.draw(screen)
        #顯示最長存活時間
        tool.show_text(f"easy mode: {tool.show_time_min(longest_survived_time['easy'])}", tool.Colors.BLACK, 0, 230, screen_center=True)
        tool.show_text(f"normal mode: {tool.show_time_min(longest_survived_time['normal'])}", tool.Colors.BLACK, 0, 290, screen_center=True)
        tool.show_text(f"hard mode: {tool.show_time_min(longest_survived_time['hard'])}", tool.Colors.BLACK, 0, 350, screen_center=True)
        tool.show_text(f"super hard mode: {tool.show_time_min(longest_survived_time['super_hard'])}", tool.Colors.BLACK, 0, 410, screen_center=True)
        tool.show_text(f"crazy mode: {tool.show_time_min(longest_survived_time['crazy'])}", tool.Colors.BLACK, 0, 470, screen_center=True)
        ...
        crazy_button = tool.text_button("select" if has_buy_crazy else crazy_btn_text, tool.Colors.BLACK, tool.Colors.PURPLE, 70, 450, 130, 50)
        # --- 繪製箭頭 ---
        if right_img_loaded:
            screen.blit(right_img_surface, right_rect)
        else:
    ...
    #存檔專區
        ...
        tool.show_text("We will save this file while you leave", tool.Colors.WHITE, 0, 140, size=24, screen_center=True)
        save_button = tool.text_button("Save File", tool.Colors.BLACK, tool.Colors.PINK, 0, 210, 200, 60, b_center=True)
        load_button = tool.text_button("Load File", tool.Colors.WHITE, tool.Colors.BLUE2, 0, 290, 200, 60, b_center=True)
        save_mode_button = tool.text_button("Game Save Mode:", tool.Colors.WHITE, tool.Colors.BLACK, 0, 370, 350, 100, t_y=400, b_center=True)
        ...
        if from_pause:
            ...
            back_button_text = "back to menu"
        back_button = tool.text_button(back_button_text, tool.Colors.WHITE, tool.Colors.ORANGE, 0, 490, 200, 60, b_center=True)
        # --- 繪製箭頭 ---
        if left_img_loaded:
            screen.blit(left_img_surface, left_rect)
        else:
    ...
    #玩家皮膚購買與更換
        ...
        display_points = "%g" % round(total_points, 1)
        tool.show_text(f"Coins:{display_points}$", tool.Colors.WHITE, 10, 120, size=28, screen_center=True)
        if from_pause:
            back_button_text = "back to pause"
        else:
            back_button_text = "back to menu"
        ...
        for name, info in unlocked_locks.items():
            # 從 player_skins 抓取對應的資料
            skin_val = player_skins[name]
                ...
                for name, info in unlocked_locks.items():
                    ...
                    if "rect" in info and info["rect"].collidepoint(mouse_pos) and not from_pause:
                        ...
                        if skin_val["has_bought"]:
                            ...
                            current_player_color_name = name
                        ...
                        # 情況 B：還沒買過 -> 判斷錢夠不夠購買
                            ...
                            if total_points >= skin_val["value"]:
                                ...
                                current_player_color_name = name  # <-- 這裡也要加，確保買完功能立刻生效
                            else:
                                # 如果錢不夠，可以加個音效或提示
    ...
    #--------------------------遊戲資料儲存與匯入--------------------------------
        ...
        cancal_button = tool.text_button("Cancel", tool.Colors.WHITE, tool.Colors.RED_2, 0, 400, 200, 60, b_center=True, show=not saved)
        ...
        if passed_time < 4000:
            ...
            tool.show_text("File Name: save_game.json", tool.Colors.BLACK, 0, 200, 50, screen_center=True)
        ...
        cancal_button = tool.text_button("Cancel", tool.Colors.WHITE, tool.Colors.RED_2, 0, 400, 200, 60, b_center=True, show=not loaded)
        # 確保有啟動計時器 (如果 collision_time 是 None)
        if tool.collision_time is None:
            tool.collision_time = pygame.time.get_ticks()
        current_time_sec = tool.sec_timer(update=True)
        ...
        if passed_time < 3000:
            ...
            if not loaded:
                ...
                reset_game()
                loaded = True
            tool.show_text("Successfuly Loaded File" if loaded_data_success else "No Save File Found, Starting New Game.", tool.Colors.BLACK, 0, 150, 50, screen_center=True)
    ...
    # 升級列表
        ...
        for i, (key, info) in enumerate(upgrade_hub_layout.items()):
            ...
            if -80 < y < HEIGHT:
                # 繪製按鈕
                rect = tool.text_button(info["title"], tool.Colors.BLACK, info["color"], 0, y, 500, 80, size=26, b_center=True)
        ...
        back_button = tool.text_button("BACK TO MENU", tool.Colors.WHITE, tool.Colors.ORANGE, 0, HEIGHT-70, 260, 50, b_center=True)
        tool.text_button("Upgrade Center", tool.Colors.WHITE, tool.Colors.BLACK, 0, 0, 500, 100, size=50, b_center=True)
        coin_rect() 
        for event in events:
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                if back_button.collidepoint(mouse_pos):
    ...
    # ------------------------------------------------------------------
        ...
        tool.show_text(f"Level: Lv.{lvl + 1}", tool.Colors.WHITE, 0, 300, size=40, screen_center=True)
        tool.show_text(f"Balance: {round(total_points, 1)}$", tool.Colors.WHITE, 0, 350, size=35, screen_center=True)
        ...
        tool.show_text(f"Effect: {display_text}", tool.Colors.WHITE, 0, 400, size=25, screen_center=True)
        # --- 萬能數值顯示邏輯 (結束) ---
        ...
        # 左箭頭：不是第一頁才顯示
        if current_p_num > 1:
            if left_img_loaded: screen.blit(left_img_surface, left_rect)
            else: pygame.draw.rect(screen, tool.Colors.RED, left_rect)
            
        # 右箭頭：不是最後一頁才顯示
        if current_p_num < total_pages:
            if right_img_loaded: screen.blit(right_img_surface, right_rect)
            else: pygame.draw.rect(screen, tool.Colors.RED, right_rect)
        ...
        if lvl < len(costs):
            ...
            if upgrade_button.collidepoint(mouse_pos):
                if total_points >= cost:
                    btn_text = f"Buy! Left ${round(total_points - cost, 1)}"
                    btn_color = tool.Colors.GREEN
                else:
                    btn_text = f"Need: ${round(cost - total_points, 1)}"
                    btn_color = tool.Colors.RED
            else:
                btn_text = f"Cost: ${cost}"
        ...
        back_button = tool.text_button("Back to Menu", back_btn_t_color, back_btn_color, 0, 500, 200, 60, b_center=True)

        # 5. 事件處理 (點擊與切換)
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if back_button.collidepoint(mouse_pos): is_pressing[0] = True
                if upgrade_button.collidepoint(mouse_pos): is_pressing[1] = True
                if left_rect.collidepoint(mouse_pos): is_pressing[2] = True
                if right_rect.collidepoint(mouse_pos): is_pressing[3] = True
            ...
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                # 執行購買
                if is_pressing[1] and upgrade_button.collidepoint(mouse_pos) and cost is not None:
                    if total_points >= cost:
                        total_points -= cost
                        current_levels[game_state] += 1 # 🔥 更新等級字典
                        if save_mode == "upgrade_save": save_data() # 儲存
                        print(f"Upgraded {game_state} to Lv.{current_levels[game_state]+1}")

                # 左切換
                if is_pressing[2] and left_rect.collidepoint(mouse_pos) and current_p_num > 1:
                    game_state = f"upgrade_p{current_p_num - 1}"
                    
                # 右切換
                if is_pressing[3] and right_rect.collidepoint(mouse_pos) and current_p_num < total_pages:
                    game_state = f"upgrade_p{current_p_num + 1}"
    ...
    #倒數前五秒
        ...
        tool.show_text(countdown_text, tool.Colors.WHITE, 0, HEIGHT // 2, screen_center=True, size=300)
    ...
    #遊戲暫停
        ...
        tool.screen_vague(10)  # type: ignore
        passed_time = pygame.time.get_ticks() - tool.collision_time if tool.collision_time is not None else 0
        countdown = 5 - (passed_time // 1000) # 倒數 3 秒
        ...
        resume_button = tool.text_button("Resume", tool.Colors.WHITE, tool.Colors.BROWN, 0, 170, 180, 60, b_center=True)
        settings_button = tool.text_button("Settings", tool.Colors.BLACK, tool.Colors.GREEN, 0, 250, 180, 60, b_center=True)
        restart_button = tool.text_button("Restart", tool.Colors.BLACK, tool.Colors.YELLOW, 0, 330, 180, 60, b_center=True)
        menu_button = tool.text_button("Back to Menu", tool.Colors.BLACK, tool.Colors.PURPLE, 0, 410, 180, 60, b_center=True)
    ...
    #死亡
        ...
        tool.show_text(f"You survive for {tool.show_time_min(current_time_sec)}", tool.Colors.WHITE, 0, 100, size=48, screen_center=True)
        gm_text = game_mode.replace("_", " ")
        tool.show_text(f"in {gm_text} mode.", tool.Colors.WHITE, 0, 150, size=48, screen_center=True)
        tool.show_text("Congratulations!" if current_time_sec >= (50 / gm_points_buff) else "Try it again!", tool.Colors.WHITE, 0, 230, size=48, screen_center=True)
        display_points = "%g" % round(points, 1)
        tool.show_text(f"points:{display_points}$", tool.Colors.WHITE, 0, 300, screen_center=True)
        tool.show_text(f"Back to Menu in {countdown} sec", tool.Colors.WHITE, 0, 410, size=40, screen_center=True)
        back_button = tool.text_button("Back to Menu", tool.Colors.WHITE, tool.Colors.ORANGE, 0, 490, 150, size=24, b_center=True)
    ...
    # 1.AFK_error
        ...
        tool.show_text("CRITICAL ERROR", tool.Colors.RED, 0, 150, size=60, screen_center=True, font_type="None")
        tool.show_text("AFK_DETECTION_TIMEOUT", tool.Colors.WHITE, 0, 240, size=25, screen_center=True, font_type="None")
        tool.show_text("Error code: 1011451", tool.Colors.GRAY, 0, 280, size=25, screen_center=True, font_type="None")
        
        # 返回主選單按鈕 - 改成亮紅色背景增加緊張感
        close_button = tool.text_button("TERMINATE PROCESS", tool.Colors.WHITE, tool.Colors.RED, 0, 400, 350, 60, b_center=True, font_type="None")
    ...
    # 2.save_game_too_old_error
        ...
        tool.show_text("SAVE_FILE_ERROR", tool.Colors.RED, 0, 150, size=55, screen_center=True, font_type="None")
        tool.show_text("YOUR 'save_game.json' IS TOO OLD", tool.Colors.WHITE, 0, 240, size=25, screen_center=True, font_type="None")
        tool.show_text("Error code: 4215788", tool.Colors.GRAY, 0, 280, size=25, screen_center=True, font_type="None")
        update_button = tool.text_button("Update your 'save_game.json'", tool.Colors.WHITE, tool.Colors.RED, 0, 400, 350, 60, b_center=True, font_type="None")
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
    ...
    # 3.game_state_error
        ...
        tool.draw_rect(tool.Colors.RED, 0, 100, 550, 450, center=True)
        pygame.draw.rect(screen, tool.Colors.BLACK2, (WIDTH//2-270, 95, 550, 450))
        tool.show_text("SOMTHING WENT WRONG", tool.Colors.RED, 0, 150, size=55, screen_center=True, font_type="None")
        tool.show_text("GAME_STATE_NOT_CORRECT", tool.Colors.WHITE, 0, 240, size=25, screen_center=True, font_type="None")
        tool.show_text("Error code: 2487145", tool.Colors.GRAY, 0, 280, size=25, screen_center=True, font_type="None")
        menu_button = tool.text_button("Back To Menu", tool.Colors.WHITE, tool.Colors.RED, 0, 400, 350, 60, b_center=True, font_type="None")
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if menu_button.collidepoint(mouse_pos):
                    is_pressing[0] = True
